...
baseline; the stack height is proved from datasheets
...
hand-soldered by the builder, not ordered from or assembled by JLCPCB.** The
connector-gender item is therefore an assembly note, not an ordering blocker.
Read POWER.md section 9 before ordering.
...
Machine-readable list: **`hand-solder.csv`** (one row per reference). Policy and
rationale: **`hand-solder.json`**.
...
**The socket gender requirement is not optional.** All 27 module boards carry
their power and ground connection as a **male** 1x05 2.54 mm header
(`Conn_01x05_Pin`) on the bottom side, so the base must carry **female** sockets.
...
|---|---|---|---|---|---|
| Slot sockets (26 pcs) | **C2897368** | `PM254-1-05-Z-8.5` (HCTL) | 8.5 mm insulator, 11.7 mm overall, square holes, top entry, 3 A | 6 155 | $0.1168 @5+ / $0.0826 @150+ |
...
Minimum socket insulator height: **6.5 mm** (proved in
`../verification/stack-height.json`). The `5V14` 2x05 socket needs a **straight**
2x05 female part — `C2897425 / PM254-2-05-W-8.5` is the right-angle family
member and must not be substituted blindly.
...
### 5. Per-board cost estimate (structure, not a quote)

Verified anchors from this pass (LCSC tier prices, 2026-09-17): the 26 socket
pieces ≈ **$2.15/board** and the 12 shunts ≈ **$0.15/board**. Every other unit
price — the 32 BOM rows, the bare 2-layer PCB
(223.52 × 160.02 mm = 357.6 cm²), the 119-placement SMT assembly charge and its
setup fee — must come from the JLCPCB quote for the chosen quantity. An estimate
produced here without those live prices would be fabricated, so this file gives
the quantities (119 placements / 32 rows / 163 refs) needed to request the quote
and states only the two prices that were actually verified. The two dominant
cost drivers to expect are the large board area and the 12 extended-part
TPS2111APWRs.
