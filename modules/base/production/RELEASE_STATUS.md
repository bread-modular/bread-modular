...
### 5. Per-board cost estimate (components, sourced)

`cost-estimate.json` is a reproducible estimate generated by
`../tools/estimate_cost.py`: it prices every part in the JLCPCB BOM and both
builder-supplied hand-solder parts from live LCSC tier prices, at a stated batch
size, attrition and tier policy. Re-run with `--refresh` to re-fetch prices.
...
| Builder-supplied hand-solder parts (25 x C2897368 + 12 x C5664) | **1.95** |
| **Components total** | **21.89** |

The single dominant item is **TPS2111APWR `C471060` at ≈$16.45/board**
(12 pieces x ≈$1.37). **Not included and only obtainable from a real quote:**
bare 2-layer PCB (223.52 × 160.02 mm = 357.6 cm²), the SMT assembly and setup
charge for 119 placements, stencil, shipping and taxes. `FB1/FB2` (`C12389`)
could not be priced from its LCSC page layout and is therefore missing from the
total (≈$0.05/board at the previously documented $0.0223 @100), so treat 21.89 as
a floor, not a quote.
...
* `cost-estimate.json` is the sourced component estimate of section 5, including
  the per-part LCSC price, tier and stock snapshot it was computed from.
