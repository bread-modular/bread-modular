...
void all_pass_filter_init(all_pass_filter_t* filter, size_t max_delay_length, float feedback) {
...
all_pass_filter_t allpass3;
all_pass_filter_t allpass4;

void set_rt60(float rt60) {
    // g = 10^(-3*(d/Fs)/RT60)
    float common = (-3.0 / 44100.0) / rt60;
    comb_filter_t* filters[] = {&comb1, &comb2, &comb3, &comb4};
    for (int lc=0; lc<4; lc++) {
        comb_filter_t* filter = filters[lc];
        float feedback = powf(10.0f, common * filter->delay_length.target_value);
        bm_param_set(&filter->feedback, feedback);
    }
}
...
static void on_midi_cc(uint8_t channel, uint8_t control, uint8_t value) {
    ...
    if (control == BM_MCC_BANK_A_CV2) {
        // size_t delay_length = bm_utils_map_range(value, 0.0, 127.0, 50.0, 400.0);
        // allpass1.delay_length = delay_length;
        // allpass2.delay_length = delay_length + delay_length * 0.1;
        return;
    }
}
...
inline static void process_audio(size_t n_samples, const int16_t* input, int16_t* output) {
    for (int lc=0; lc<n_samples; lc += 2) {
        ...
        allpass_processed = all_pass_filter_process(&allpass3, allpass_processed);
        allpass_processed = all_pass_filter_process(&allpass4, allpass_processed);

        float processed = allpass_processed;
...
static void init(bm_app_host_t host) {
    ...
    all_pass_filter_init(&allpass1, 142, 0.80);
    all_pass_filter_init(&allpass2, 56, 0.80);
    all_pass_filter_init(&allpass3, 280, 0.80);
    all_pass_filter_init(&allpass4, 400, 0.80);
}
...
static void destroy() {
    ...
    all_pass_filter_destroy(&allpass3);
    all_pass_filter_destroy(&allpass4);
}
