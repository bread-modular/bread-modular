# ATtiny1616 HelloWorld Firmware — no PlatformIO / VSCode
...
A demo / template firmware for the **ATtiny1616** (microchip megaAVR 0-series).

- Receives MIDI over the hardware UART (`Serial`) and parses it with the local
  `SimpleMIDI` library.
- Drives a gate pin high/low on note on/off.
- Reads two CV inputs (`PIN_CV1`, `PIN_CV2`) and mixes them into the 8-bit
  DAC output (`DAC0`), scaled to 0–127.
- Supports a 3-position mode switch with debouncing (`ModeHandler`) and
  LED toggle feedback (`LEDToggler`).
...
```
helloworld/
├── helloworld/           ← Arduino sketch (THE source to edit)
│   ├── helloworld.ino    ← main firmware
...
> **Edit `helloworld/helloworld.ino` and its headers.** The build/flash scripts
> live in `opt/attiny1616-tools/` (shared across Bread Modular modules) and are
